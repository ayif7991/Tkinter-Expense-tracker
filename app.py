...
class ExpenseTracker:
    ...
    def added(self, boxaile):
        myLabel = Label(boxaile, text="value inserted")
        myLabel.grid(row=5, column=0)

    def delete(self, boxaile):
        myLabel = Label(boxaile, text="value deleted")
        myLabel.grid(row=5, column=0)
    ...
    def insert(self, database, val1, val2, val3):
        ...
        price = val2.get()
        date = val3.get()
        insertion = database(item, price, date)
        return insertion

    def find_expense(self, database, val1, val2):
        item = val1.get()
        price = val2.get()
        find = database(item.price)
        return find

    def delete_expense(self, database, val1, val2):
        item = val1.get()
        price = val2.get()
        delete = database(item, price)
        return delete
    ...
    def main_window(self):
        ...
        button3 = Button(self.frame, text="Entertainment expense", command=self.entertainment)
        button3.pack()

        button4 = Button(self.frame, text="Other expense", command=self.other)
        button4.pack()
    ...
    def groceries(self):
        ...
        B1 = Button(top, text="Insert Values",
                    command=lambda: (self.insert(db.insert_groceries, e1, e2, e3), self.added(top)))
        B1.grid(row=1, column=2)

        B2 = Button(top, text="Select All", command=lambda: (
            text.delete(1.0, END), text.insert(END, self.display_all(db.select_all_groceries()))))
        B2.grid(row=2, column=2)

        B3 = Button(top, text="Find value", command=lambda: (
            text.delete(1.0, END), text.insert(END, self.find_expense(db.select_grocery, e1, e2))))
        B3.grid(row=2, column=3)

        B3 = Button(top, text="Delete expense",
                    command=lambda: (self.delete_expense(db.delete_grocery, e1, e2), self.delete(top)))
    ...
    def household(self):
        ...
        B2 = Button(top, text="Select All", command=lambda: (
            text.delete(1.0, END), text.insert(END, self.display_all(db.select_all_household()))))
        B2.grid(row=2, column=2)
